  ...
  group('Performance Benchmarks', () {
    ...
    group('Processing speed requirements', () {
      test('processes 2000px image in under 3 seconds', () async {
        ...
        expect(
          ...
          lessThan(3.0),
          reason:
              'Processing took ${elapsedSeconds.toStringAsFixed(2)}s, should be under 3s',
        );

        print(
          '2000px image processed in ${elapsedSeconds.toStringAsFixed(2)}s',
        );
      });
      ...
      test('handles multiple sequential processes efficiently', () async {
        ...
        print(
          'Average processing time for $processCount processes: ${averageTime.toStringAsFixed(2)}ms',
        );
      });
    ...
    group('Image size optimization verification', () {
      ...
      test('processed images maintain reasonable quality', () async {
        ...
        final processedImageData = await imageProcessor.processImage(
          originalImageData,
          options,
        );
        ...
        print(
          'Compression ratio: ${(processedImageData.length / originalImageData.length * 100).toStringAsFixed(1)}%',
        );
      });
    });
  });
...
Uint8List _createTestImage(int width, int height) {
  ...
  for (int i = 0; i < 5; i++) {
    for (int x = 0; x < width; x++) {
      ...
      if (startY + contentHeight - i >= 0 &&
          startY + contentHeight - i < height) {
        image.setPixel(
          x,
          startY + contentHeight - i,
          img.ColorRgb8(100, 100, 100),
        );
      }
    }
